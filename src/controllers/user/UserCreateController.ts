...
class UserCreateController {
    ...
    @httpPost('/')
    async execute(@requestBody() data: ICreateUser, @response() res: Response): Promise<ICreateUserReturn | Response> {
        const userCreate = await this.createUserService.execute(data);
        return res.send({ name: userCreate?.name, status: StatusCode.Created, message: 'Usuário criado!' });
    }
}
